...
from deepemotions.deepcluster import DeepKMeans
...
for s in subjects:
    ...
    labels_hat, time_hat = deepkmeans.run(nb_classes=NB_CLASSES, seq_len=SEQ_LEN, s_init=s_init, epochs_pre=1, epochs_train=1, batch_size=128, 
            embedding=30, lr_pretrain=5e-5, lr_train=5e-5,  pre_train_cluster=True)
    membership = deepkmeans.membership(m=2)
    print(membership)
    print("there is nan in labels", np.isnan(labels_hat).any())
